...
| `src/app/lenker.ts`                   | App-lokale eksterne lenker                                | Migrer selve lenkene til `@sif/soknad-ui/lenker`; behold gjerne `src/app/lenker.ts` som en tynn app-adapter for ergonomi (`useLenker`, `getLenke`) |
