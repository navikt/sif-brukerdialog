...
export const rapporterInntektUngdomsprogramytelse = async (
    data: ungdomsytelse.UngdomsytelseInntektsrapportering,
): Promise<void> => {
    try {
        await ungdomsytelse.UngdomsytelseController.inntektrapportering({ body: data, headers: commonRequestHeader });
    } catch (e) {
        throw handleApiError(e, 'rapporterInntektUngdomsprogramytelse');
    }
};
