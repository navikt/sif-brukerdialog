import { QuestionConfig, Questions, questionValueIsOk, QuestionVisibility } from '../';
...
describe('question', () => {
    ...
    it('returns all initially included questions', () => {
        const visibility = getVisibility(testPayload);
        expect(visibility.getIncludedQuestions().length).toEqual(2);
    });
    it('returns all included questions when allowChildren is trur', () => {
        const visibility = getVisibility({ ...testPayload, allowChildren: true });
        expect(visibility.getIncludedQuestions().length).toEqual(4);
    });
});
