...
export const durationToISODuration = (duration: NumberDuration | Partial<Duration>): ISODuration => {
    ...
    return `PT${hours}H${minutes}M` as ISODuration;
};
...
export const getDatesWithDurationLongerThanZero = (duration: DateDurationMap | InputDateDurationMap): ISODate[] =>
    (Object.keys(duration) as ISODate[]).filter((date) => {
        const d = duration[date];
        return isValidDuration(d) && !durationIsZero(ensureDuration(d));
    });
/**
...
export const getDurationsInDateRange = (dateDurationMap: DateDurationMap, dateRange: DateRange): DateDurationMap => {
    ...
    (Object.keys(dateDurationMap) as ISODate[]).forEach((isoDate) => {
        if (isDateInDateRange(isoDate, dateRange)) {
...
export const removeDatesFromDateDurationMap = (
    ...
    datesToRemove: ISODate[],
): DateDurationMap => {
    const datesToRemoveSet = new Set(datesToRemove);

    const cleanedMap: DateDurationMap = {};
    (Object.keys(dateDurationMap) as ISODate[]).forEach((key) => {
        if (!datesToRemoveSet.has(key)) {
...
export const getDateRangeFromDateDurationMap = (dateDurationMap: DateDurationMap): DateRange => {
    const unsortedDates = Object.keys(dateDurationMap) as ISODate[];
