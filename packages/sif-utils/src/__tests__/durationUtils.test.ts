...
describe('durationUtils', () => {
    ...
    describe('getDurationsDiff', () => {
        it('removes equal values', () => {
            const result = getDurationsDiff(
                { ['2021-01-01' as ISODate]: { hours: '1', minutes: '2' } },
                { ['2021-01-01' as ISODate]: { hours: '1', minutes: '2' } },
            );
        ...
        it('returns changed values', () => {
            const result = getDurationsDiff(
                { ['2021-01-01' as ISODate]: { hours: '1', minutes: '2' } },
                { ['2021-01-01' as ISODate]: { hours: '2', minutes: '2' } },
            );
            expect(Object.keys(result).length).toBe(1);
        });
        it('returns new values', () => {
            const result = getDurationsDiff({ ['2021-01-01' as ISODate]: { hours: '1', minutes: '2' } }, {});
            expect(Object.keys(result).length).toBe(1);
        });
    });
    ...
    describe('removeInvalidDurations', () => {
        it('removes duration which has duration with undefined hours and minutes', () => {
            const result = getValidDurations({
                ['2021-02-05' as ISODate]: { hours: undefined, minutes: undefined },
            });
            expect(Object.keys(result).length).toBe(0);
        });
        it('removes duration which has duration with invalid values in hours or minutes', () => {
            const result = getValidDurations({
                ['2021-02-05' as ISODate]: { hours: 'a', minutes: '0' },
            });
            expect(Object.keys(result).length).toBe(0);
        });
        it('does not remove duration which is valid', () => {
            expect(
                Object.keys(
                    getValidDurations({
                        ['2021-02-05' as ISODate]: { hours: '0', minutes: '0' },
                    }),
                ).length,
            ).toBe(1);
            expect(
                Object.keys(
                    getValidDurations({
                        ['2021-02-05' as ISODate]: { hours: '', minutes: '0' },
                    }),
                ).length,
            ).toBe(1);
            expect(
                Object.keys(
                    getValidDurations({
                        ['2021-02-05' as ISODate]: { hours: '1', minutes: undefined },
                    }),
                ).length,
            ).toBe(1);
        });
    ...
    describe('summarizeDateDurationMap', () => {
        it('sums correctly', () => {
            const result = summarizeDateDurationMap({
                ['2021-01-01' as ISODate]: { hours: '1', minutes: '2' },
                ['2021-01-02' as ISODate]: { hours: '1', minutes: '2' },
                ['2021-01-03' as ISODate]: { hours: '1', minutes: '2' },
                ['2021-01-04' as ISODate]: { hours: '1', minutes: '2' },
                ['2021-01-05' as ISODate]: { hours: '1', minutes: '2' },
            });
    ...
    describe('getDatesWithDurationLongerThanZero', () => {
        it('includes date with minutes', () => {
            expect(
                getDatesWithDurationLongerThanZero({ ['2021-01-01' as ISODate]: { minutes: '2', hours: undefined } })
                    .length,
            ).toBe(1);
        });
        it('includes date with hours', () => {
            expect(
                getDatesWithDurationLongerThanZero({ ['2021-01-01' as ISODate]: { hours: '1', minutes: undefined } })
                    .length,
            ).toBe(1);
        });
        it('excludes date with 0 minutes and 0 hours', () => {
            expect(
                getDatesWithDurationLongerThanZero({ ['2021-01-01' as ISODate]: { hours: '0', minutes: '0' } }).length,
            ).toBe(0);
        });
        it('excludes date with invalid duration', () => {
            expect(
                getDatesWithDurationLongerThanZero({ ['2021-01-01' as ISODate]: { hours: 'a', minutes: '0' } }).length,
            ).toBe(0);
            expect(
                getDatesWithDurationLongerThanZero({ ['2021-01-01' as ISODate]: { hours: '0', minutes: 'a' } }).length,
            ).toBe(0);
            expect(
                getDatesWithDurationLongerThanZero({
                    ['2021-01-01' as ISODate]: { hours: undefined, minutes: undefined },
                }).length,
            ).toBe(0);
        });
    });
    describe('getDateDurationDiff', () => {
        ...
        it('returns new values', () => {
            const result = getDateDurationDiff({ ['2021-01-01' as ISODate]: { hours: '1', minutes: '2' } }, {});
            expect(Object.keys(result).length).toBe(1);
        });
    });
    ...
    describe('getDurationsInDateRange', () => {
        ...
        it('returns only dates within the date range', () => {
            const result = getDurationsInDateRange(data, {
                from: '2021-01-02' as ISODate,
                to: '2021-01-03' as ISODate,
            });
    ...
    describe('getPercentageOfISODuration', () => {
        it('keeps 100% of 1 hour 30 minutes the same', () => {
            expect(getPercentageOfISODuration('PT1H30M' as ISODuration, 100)).toEqual('PT1H30M');
        });
        it('returns half 45 minutes when calculating 50% of 1 hour 30 minutes', () => {
            expect(getPercentageOfISODuration('PT1H30M' as ISODuration, 50)).toEqual('PT0H45M');
        });
        it('returns undefined if ISODuration is not valid ISODuration', () => {
            expect(getPercentageOfISODuration('P1XH30M' as ISODuration, 50)).toBeUndefined();
        });
    });
    ...
    describe('removeDatesFromDateDurationMap', () => {
        const keyDateBefore = '2020-01-02' as ISODate;
        const keyDate1 = '2020-01-03' as ISODate;
        const keyDate2 = '2020-01-04' as ISODate;
        const keyDate3 = '2020-01-05' as ISODate;
        const keyDateAfter = '2020-01-06' as ISODate;
        ...
        it('keeps dates which are in array of dates to be removed', () => {
            const result = removeDatesFromDateDurationMap(dateMap, [keyDate1]);
            expect(Object.keys(result).length).toBe(2);
        ...
        it('keeps all dates id removeDates is only other dates', () => {
            const result = removeDatesFromDateDurationMap(dateMap, [keyDateBefore, keyDateAfter]);
            expect(Object.keys(result).length).toBe(3);
            expect(result[keyDate1]).toBeDefined();
            expect(result[keyDate2]).toBeDefined();
            expect(result[keyDate3]).toBeDefined();
    ...
    describe('getDateRangeFromDateDurationMap', () => {
        it('gets correct date range from map with one date', () => {
            const result = getDateRangeFromDateDurationMap({ ['2022-01-03' as ISODate]: { hours: '1', minutes: '0' } });
            expect(result.from).toEqual('2022-01-03' as ISODate);
            expect(result.to).toEqual('2022-01-03' as ISODate);
        });
        it('gets correct date range from map with two dates', () => {
            const result = getDateRangeFromDateDurationMap({
                ['2022-01-03' as ISODate]: { hours: '1', minutes: '0' },
                ['2022-01-04' as ISODate]: { hours: '1', minutes: '0' },
            });
            expect(result.from).toEqual('2022-01-03' as ISODate);
            expect(result.to).toEqual('2022-01-04' as ISODate);
        });
        it('gets correct date range from map with multiple dates in wrong order', () => {
            const result = getDateRangeFromDateDurationMap({
                ['2022-01-04' as ISODate]: { hours: '1', minutes: '0' },
                ['2022-01-05' as ISODate]: { hours: '1', minutes: '0' },
                ['2022-01-03' as ISODate]: { hours: '1', minutes: '0' },
            });
            expect(result.from).toEqual('2022-01-03' as ISODate);
            expect(result.to).toEqual('2022-01-05' as ISODate);
        });
    });
