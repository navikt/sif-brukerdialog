import { oppgavebekreftelse_nb as felles } from '../../../oppgavebekreftelse/i18n/nb';

export const andreLivsoppholdsytelserMessages_nb = {
    '@ungInnsyn.oppgavetype.BEKREFT_ANDRE_LIVSOPPHOLDSYTELSER.paneltittel':
        felles['@ungInnsyn.oppgavebekreftelse.felles.paneltittel'],
    '@ungInnsyn.oppgavetype.BEKREFT_ANDRE_LIVSOPPHOLDSYTELSER.oppgavetittel':
        felles['@ungInnsyn.oppgavebekreftelse.felles.oppgavetittel'],
    '@ungInnsyn.oppgavetype.BEKREFT_ANDRE_LIVSOPPHOLDSYTELSER.info':
        felles['@ungInnsyn.oppgavebekreftelse.felles.info'],
    '@ungInnsyn.oppgavetype.BEKREFT_ANDRE_LIVSOPPHOLDSYTELSER.harTilbakemeldingSpørsmål':
        felles['@ungInnsyn.oppgavebekreftelse.felles.harTilbakemeldingSpørsmål'],
    '@ungInnsyn.oppgavetype.BEKREFT_ANDRE_LIVSOPPHOLDSYTELSER.harIkkeUttalelseLabel':
        felles['@ungInnsyn.oppgavebekreftelse.felles.harIkkeUttalelseLabel'],
    '@ungInnsyn.oppgavetype.BEKREFT_ANDRE_LIVSOPPHOLDSYTELSER.harUttalelseLabel':
        felles['@ungInnsyn.oppgavebekreftelse.felles.harUttalelseLabel'],
    '@ungInnsyn.oppgavetype.BEKREFT_ANDRE_LIVSOPPHOLDSYTELSER.tilbakemeldingFritekstLabel':
        felles['@ungInnsyn.oppgavebekreftelse.felles.tilbakemeldingFritekstLabel'],
    '@ungInnsyn.oppgavetype.BEKREFT_ANDRE_LIVSOPPHOLDSYTELSER.kvitteringTekst':
        felles['@ungInnsyn.oppgavebekreftelse.felles.kvitteringTekst'],
};
