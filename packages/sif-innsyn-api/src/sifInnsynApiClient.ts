import { EnvKey, getSifInnsynBrowserEnv } from '@navikt/sif-common-env';
import { AxiosError } from 'axios';
import { v4 } from 'uuid';

import { client } from './sif-innsyn-api/client/client.gen';
...
export const initSifInnsynApiClient = (options?: InitOptions) => {
    const apiBaseUrl = (typeof window !== 'undefined' && window.location.origin) || '';
    /** Set config for generert klient */
    client.setConfig({
        withCredentials: false,
        headers: options?.headers,
        baseURL: `${apiBaseUrl}${getSifInnsynBrowserEnv()[EnvKey.SIF_INNSYN_FRONTEND_PATH]}`,
    });
