...
import {
  mapBarnKalkulatorToBarn,
  summerAntallOmsorgsdager,
} from "@/utils/utils";
...
} from "../../components/result/ResultView";
...
const Kalkulator = () => {
  const intl = useIntl();
  const [openBarnPanel, setOpenBarnPanel] = useState<boolean | undefined>(
    undefined
  );
  const [resultViewData, setResultViewData] =
    useState<ResultView<number>>(empty);
  ...
  const onValidSubmit = (values: Partial<KlakulatorFormValues>) => {
    ...
    const omsorgsprinsipper: Omsorgsprinsipper = beregnOmsorgsdager(
      mappedBarn,
      inkluderKoronadager
    );
