...
export const erForbiDetTolvteKalenderår = (årFødt: number): boolean =>
  getYear() - årFødt > 12;

export const erForbiDetAttendeKalenderår = (årFødt: number): boolean =>
  getYear() - årFødt > 18;

export const barnetErForbiDetTolvteKalenderårOgIkkeKroniskSykt = (
  barn: BarnKalkulator
): boolean =>
  erForbiDetTolvteKalenderår(barn.årFødt) &&
  barn.kroniskSykt !== undefined &&
  barn.kroniskSykt === YesOrNo.NO;
...
export const includeChild = (barn: BarnKalkulator): boolean =>
  !excludeChild(barn);

export const summerAntallOmsorgsdager = (result: Omsorgsprinsipper): number => {
  const { grunnrett, kroniskSykt, aleneomsorg, aleneomsorgKroniskSyke } =
    result;
  return (
