...
test('Fyll ut omsorgstilbud', async ({ page }) => {
    await page
        .getByRole('radiogroup', { name: 'Har barnet vært fast og' })
        .getByLabel('Ja, i hele eller deler av')
        .click();
    await page.getByRole('radiogroup', { name: 'Har barnet vært fast og' }).getByLabel('Nei').click();
    ...
    await page.getByLabel('Timer', { exact: true }).click();
    await page.getByLabel('Timer', { exact: true }).press('Shift+ArrowRight');
    await page.getByLabel('Timer', { exact: true }).fill('2');
    ...
    await page.getByLabel('Timer', { exact: true }).fill('5');
    await page.getByRole('button', { name: 'Lagre' }).click();
    ...
    await page.getByLabel('Timer', { exact: true }).fill('3');
