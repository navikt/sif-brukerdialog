...
export const contextConfig = (props?: ConfigProps) => {
    ...
    beforeEach('intercept mellomlagring og levere tomt objekt', () => {
        cy.intercept(`GET`, `${API}/mellomlagring/OMSORGSPENGER_UTBETALING_SNF`, mellomlagring || { noData: 1 });
        cy.intercept(`DELETE`, `${API}/mellomlagring/OMSORGSPENGER_UTBETALING_SNF`, mellomlagring || {});
        cy.intercept(`PUT`, `${API}/mellomlagring/OMSORGSPENGER_UTBETALING_SNF`, {}).as('putMellomlagring');
        cy.intercept(`POST`, `${API}/mellomlagring/OMSORGSPENGER_UTBETALING_SNF`, {});
        cy.intercept(`POST`, `${API}/omsorgspenger-utbetaling-snf/innsending`, {}).as('innsending');
        ...
        cy.intercept('GET', `${API}/oppslag/barn*`, { barn: props?.barn } || cyApiMockData.barnMock).as('getBarn');
