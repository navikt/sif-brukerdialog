...
// View-motor kun for mustache-dekor
...
const startServer = async () => {
    ...
    // Lag fil til bruk som fallback
    const decoratedHtmlPath = path.resolve(__dirname, 'index-decorated.html');
    fs.writeFileSync(decoratedHtmlPath, renderedHtml);
    ...
    server.listen(port, () => {
        console.log(`✅ Dev-server kjører på http://localhost:${port}`);
    });
};
