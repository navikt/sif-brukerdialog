...
test('Søk opp og legg til ny deltaker', async ({ page }) => {
    ...
    await page
        .locator('div')
        .filter({ hasText: /^Kopier 56857102105 - ikke registrert$/ })
        .getByRole('button')
        .click();
    await page.getByRole('textbox', { name: 'Fødselsnummer/d-nummer:' }).click();
    await page.getByRole('textbox', { name: 'Fødselsnummer/d-nummer:' }).fill('56857102105');
    await page.getByRole('textbox', { name: 'Fødselsnummer/d-nummer:' }).press('Tab');
